...
회계에서 일별 잔액은 두 컬럼으로 표현된다:
- **opening_balance (기초 잔액)**: 그 날 0시 시점 통장 잔액
- **closing_balance (기말 잔액)**: 그 날 24시 시점 통장 잔액 (= opening + net_amount)

예: 1번 계좌가 어제 끝날 때 100만원, 오늘 +30/-20만원 거래 → opening 100만, closing 110만, net +10만.

처음 설계 시 `DailyTransactionSummary`에 두 컬럼을 두려 했다. 직관적으로 합리적이지만, **"이 값을 어디서 가져올 것이냐"** 에서 막힌다.

#### 잔액의 출처 문제

우리 `transactions` 테이블에는 거래 1건당 `amount`(변동량)만 있고 **그 시점의 잔액은 없다**. 그러니 일별 잔액을 알려면 어딘가의 기준점에서 누적해야 한다. 기준점 후보 4가지를 따져 보면:

| 옵션 | 방식 | 문제 |
|---|---|---|
| A | 직전일 `daily_transaction_summaries.closing_balance` 참조 | 첫 날 opening은? 직전일 정산이 실패했으면? 의존 사슬이 길고 부서지기 쉬움 |
| B | `accounts.balance` 컬럼에서 가져오기 | 그건 *현재* 잔액. 과거 날짜 정산엔 미래 거래 영향이 이미 반영된 값이라 의미 어긋남 |
| C | `account_balance_snapshots` 테이블 신설, 매일 0시 스냅샷 | 스냅샷 자체가 또 다른 운영 인프라. 정산 배치가 떠받칠 일이 아님 |
| D | 매 거래에 `balance_after` 컬럼을 같이 기록 (running balance) | 거래 시점에 잔액 갱신할 책임은 운영 시스템(코어뱅킹). 정산 시스템은 그 권원을 갖지 않음 |

#### 왜 정산 배치의 책임이 아닌가

은행 시스템은 보통 두 층으로 나뉜다:

```
   [코어뱅킹]                            [정산/회계]
   실시간 거래 처리 + 잔액 갱신          하루치 데이터 모아 집계·장부 작성
   잔액의 source of truth                 읽기 전용 후속 시스템
```

**잔액의 권원은 코어뱅킹**에 있고, 본 프로젝트는 그걸 만드는 시스템이 아니라 **읽는 시스템**이다. 위 옵션 C/D는 모두 "정산 시스템이 잔액 추적 인프라를 직접 떠받친다"는 뜻인데, 그건 책임 분리 관점에서 잘못된 위치.

추가로 우리 시드는 한 번 더 단순하다: `accounts.balance`를 시드 시점에 1번 설정한 뒤, 거래가 발생해도 갱신하지 않는다. 즉 어떤 옵션을 골라도 시드 데이터에서 의미 있는 잔액을 못 채운다.

#### 결론

opening/closing balance 컬럼을 두면 무엇을 채워도 **잘못된 값**이거나 **검증 불가능한 값**이 들어간다. 컬럼이 있으면 누군가 "여기 잔액이네" 하고 신뢰해버릴 위험. 그러느니 컬럼 자체를 빼는 게 정직하다. 진짜 잔액 추적이 필요해지면 별도 모듈(`account_balance_snapshots` 테이블 + 그걸 채우는 별도 잡)로 추가하는 게 맞다.

> **한 문장 요약**: 잔액의 권원은 코어뱅킹에 있고 정산 배치는 그걸 만드는 시스템이 아니다. 우리 데이터엔 권원 정보가 없으므로 컬럼을 정직하게 채울 방법이 없어 드랍.
